...
# This chunk shows one example where combined regression outperforms logistic regression and three examples where combined regression outperforms transformed linear regression.

# Note: 'latent: binary variable contains all information', 'asymmetric + outliers: linear regression fails to capture all signal'

# Note: Simulating correlated features? Then use constant signs for betas!
...
for(i in seq_len(4)){
  ...
  for(j in seq_len(100)){
    ...
    # eta <- X %*% beta + stats::rnorm(n=n) # original
    eta <- X %*% beta # trial
    ...
    if(i==1){
      ...
      #y <- ifelse(eta<0,-3,3) + stats::rnorm(n=n)
      #y <- ifelse(eta<0,-stats::runif(n),+stats::runif(n))
      #y <- ifelse(eta<0,stats::rnorm(n=n,mean=-2),stats::rnorm(n=n,mean=2))
      #y <- ifelse(eta<0,stats::rnorm(n=n,mean=-1,sd=0.4),stats::rnorm(n=n,mean=1,sd=0.4))
      #y <- ifelse(eta<0,-1,+1)*abs(stats::rnorm(n)) # original
      y <- ifelse(eta<0,-2,+2)+epsilon # trial
      table(y>=0,eta>=0)
    } else if(i==3){
      #y <- ifelse(eta<0,-sqrt(abs(eta)),eta^2) # original
      y <- ifelse(eta<0,-sqrt(abs(eta+epsilon)),(eta+epsilon)^2) # trial
    } else if(i==4){
      #y <- eta + stats::rbinom(n=n,size=1,prob=0.05)*(2*stats::rbinom(n=n,size=1,prob=0.5)-1)*1.5*max(abs(eta)) # original
      y <- eta + epsilon + stats::rbinom(n=n,size=1,prob=0.05)*(2*stats::rbinom(n=n,size=1,prob=0.5)-1)*1.5*max(abs(eta)) # trial
    }
