...
XX <- sign(X)*abs(X)^rep(sample(c(1/3,1/2,2,3),size=p,replace=TRUE),each=n)
y <- XX %*% beta
test <- cv.cornet(y=y,cutoff=0,X=X,alpha=1)
test$auc
...
loss <- t(sapply(lasso,function(x) x$deviance))
prop <- loss[,"binomial"]/loss[,"gaussian"]
prop <- loss[,"combined"]/loss[,"gaussian"]
plot(prop); abline(h=1,col="grey")
which.min(prop)
frame[which.min(prop),]
```
