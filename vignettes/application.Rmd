...
#utils::install.packages(pkgs=c("devtools","missRanger","xtable","randomForest","e1071","MLmetrics"))
  ...
  loss[[i]] <- fit[[i]] <- p.log[[i]] <- p.lin[[i]] <- list()
    ...
    for(j in seq_along(names)){
      ...
      loss[[i]][[names[j]]] <- cornet::cv.cornet(y=y[cond],cutoff=25.5,
                                  X=x[[i]][cond,],alpha=alpha,rf=alpha==1,svm=alpha==1)
      set.seed(i)
...
for(i in 1:3){
  ...
  # add other methods here (svm and knn)
  ...
  table <- cbind(table,c(lasso[c("logistic lasso regression","combined lasso regression")],ridge[c("logistic ridge regression","combined ridge regression")],lasso["random forest*"]))
}
colnames(table) <- paste("year",1:3)
rownames(table) <- paste("\\small",rownames(table))
