  ...
  loss[[i]] <- fit[[i]] <- p.log[[i]] <- p.lin[[i]] <- list()
    ...
    for(j in seq_along(names)){
      ...
      fit[[i]][[names[j]]] <- cornet::cornet(y=y[cond],cutoff=25.5,
                                  X=x[[i]][cond,],alpha=alpha)
      set.seed(i)
      temp <- median(replicate(n=50,expr=cornet:::.test(y=y[cond],cutoff=25.5,X=x[[i]][cond,],alpha=alpha)))
      p.log[[i]][[names[j]]] <- temp$log
      p.lin[[i]][[names[j]]] <- temp$lin
    }
    cat("\n")
}
...
for(i in names){
  ...
  frame[i,"pval.min"] <- round(min(pvalue),digits=3) # signif digits=2
...
#digits <- 2+c(0,1*grepl(pattern="delta_|p_",x=colnames(frame))) # old
xtable <- xtable::xtable(frame) #,digits=digits) # old
...
for(i in names){
  for(j in type){
    ...
    change <- 100*(value$combined-value$binomial)/value$binomial
    #change <- 100*(value$combined-value$gaussian)/value$gaussian
    frame[i,j] <- median(change)
  }
}

#median <- format(round(median,digits=2),trim=TRUE)
#min <- format(round(min,digits=2),trim=TRUE)
#max <- format(round(max,digits=2),trim=TRUE)
#frame[,] <- paste0(median," [",min,",",max,"]")

frame <- format(round(frame,digits=1))
frame <- gsub(pattern=" ",replacement="\\\\textcolor{green}{+}",x=frame)
colnames(frame) <- sapply(colnames(frame),function(x) switch(x,class="\\textsc{mcr}",mse="\\textsc{mse}",mae="\\textsc{mae}",auc="\\textsc{auc}",x))
colnames(frame) <- paste0("$\\textcolor{green}{\\Delta}$",colnames(frame))
rownames(frame) <- paste0(substr(x=rownames(frame),start=1,stop=5)," ",
                          substr(x=rownames(frame),start=6,stop=6))
